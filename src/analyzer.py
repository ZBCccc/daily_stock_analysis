...
"""
        if context.get('asset_type') == 'fund':
            prompt += """
> ⚠️ **开放式基金分析约束**：该标的为开放式基金（非ETF）。
> - **数据特征**：基金只有净值数据，无成交量、换手率、量比等交易数据
> - **价格解读**：收盘价即为单位净值，开盘/最高/最低价均等于净值（无盘中波动）
> - **分析重点**：关注净值走势、均线系统、涨跌幅，忽略成交量相关指标
> - **操作建议**：使用"定投"/"持有"/"观望"/"赎回"等基金术语，而非"买入"/"卖出"
> - **风险因素**：关注基金经理变动、持仓风格漂移、回撤控制、费率成本
> - **严禁提及**：量比、换手率、筹码分布、涨停板等股票特有概念
